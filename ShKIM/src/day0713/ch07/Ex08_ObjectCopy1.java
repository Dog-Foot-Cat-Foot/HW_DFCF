...
public class Ex08_ObjectCopy1 {
	...
	public static void main(String[] args) {
		...
		// bookArray1 배열의 0~2번째 배열 요소에 
		// new 예약어로 생성한 인스턴스의 주소를 대입한다
		bookArray1[0] = new Book("태백산맥", "조정래");
		bookArray1[1] = new Book("데미안", "헤르만 헤세");
		bookArray1[2] = new Book("어떻게 살 것인가", "유시민");
		
		// System 객체의 arraycopy() 메서드 호출하여 배열 0번째부터 3개의 배열 요소 복사
		System.arraycopy(bookArray1, 0, bookArray2, 0, 3);
		
		// for문, bookArray2의 배열 요소 개수만큼 반복 수행
		for(int i = 0; i < bookArray2.length; i++) {
			// bookArray2의 i번째 배열요소에 들어있는 메모리 주소값으로 
		...
		for(int i = 0; i < bookArray1.length; i++) {
			// bookArray1의 i 번째 요소에 들어있는 메위주로 Book 인스턴스의 showBookInfo() 메서드 호출
		...
		for(int i = 0; i < bookArray2.length; i++) {
			// bookArray2의 i번째 요소에 들어있는 메위주로 Book 인스턴스의 showBookInfo() 메서드 호출 
			bookArray2[i].showBookInfo();
		}
