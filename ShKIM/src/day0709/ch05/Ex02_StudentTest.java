...
public class Ex02_StudentTest {
	...
	public static void main(String[] args) {
		...
		// studentKim 변수의 메모리 위치 주소값을 통해 studentName 멤버변수 접근하여
		// studentName의 멤버 변수에 "김석진" 라는 문자열의 메모리 위치 주소값 대입
		// studentKim.studentName = "김석진";
		// 오류: The field Student.studentName is not visible
		// studentName 변수의 접근 제어가 default -> private로 변경되어 외부 클래스 직접 접근 불가함
		
		// setStudentName() 메서드를 호출하여 private 변수 간접 접근
		studentKim.setStudentName("김석진");
		
		
		// studentKim 변수로 Student 객체에 접근하여 studentName 멤버 변수의 데이터 출력
		// System.out.println(studentKim.studentName);
		
		// studentKim 변수로 Student 객체에 접근하여 getStudentName() 메서드 호출하여
		// 리턴 받은 데이터를 출력
